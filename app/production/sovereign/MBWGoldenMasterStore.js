...
import { MBW_LEGAL_VERSION, MBW_PRODUCTION_SERVICE_STATUS, mbwRouteAccess } from './MBWReleaseContracts';
...
async function decryptState(envelopeText) {
  ...
  const expectedMac = CryptoJS.HmacSHA256(`${envelope.iv}.${envelope.ciphertext}`, integrityKey).toString(CryptoJS.enc.Hex);
...
function playSeepTurn(seep, cardId, ai = false) {
  ...
  const table = captured.length ? seep.table.filter(item => !capturedIds.has(item.id)) : [...seep.table, card];
  const next = {
    ...seep,
...
function advanceLudo(tokens, dice) {
  const candidates = tokens.map((token, index) => ({
    token,
    index
  })).filter(item => legalLudoToken(item.token, dice)).sort((left, right) => right.token - left.token);
...
function coinEntry(state, amount, reason) {
  return {
    ...
    ledger: [{
      ...
      reason,
      at: now()
    }, ...state.coins.ledger].slice(0, 200)
  };
}
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'GATE_FAILURE':
      {
        ...
        return withUpdated(state, {
          lifecycle: {
            ...state.lifecycle,
            lastError: lockedUntil ? 'ACCESS TEMPORARILY LOCKED' : 'ACCESS DENIED'
          },
          security: {
            ...state.security,
            gateFailures: lockedUntil ? 0 : failures,
            gateLockedUntil: lockedUntil
          }
        });
      }
    case 'ERROR':
    ...
    case 'TIER':
      {
        const receipt = {
          ...
          at: now(),
          mode: 'TIER_SELECTION_PENDING_BILLING'
        };
        return withUpdated(state, {
          subscription: {
            tier: action.tier,
          ...
          userSeed: {
            ...
            tier: action.tier,
            badge: action.badge,
            subscriptionState: 'TIER_SELECTED',
            updatedAt: now()
          }
        });
      }
    case 'CONSENT':
      {
        const acceptedAtKey = action.key === 'privacyAccepted' ? 'privacyAcceptedAt' : action.key === 'termsAccepted' ? 'termsAcceptedAt' : 'consentAcceptedAt';
        return withUpdated(state, {
          safety: {
            ...state.safety,
    ...
    case 'MATCH_SWIPE':
      {
        ...
        const matches = isMatch && !state.matchmaking.matches.some(item => item.id === profile.id) ? [...state.matchmaking.matches, profile] : state.matchmaking.matches;
        return withUpdated(state, {
          matchmaking: {
            ...state.matchmaking,
            cursor,
            ...
            passed,
            matches,
            selectedMatchId: isMatch ? profile.id : state.matchmaking.selectedMatchId
          }
        });
      }
    case 'SELECT_MATCH':
    ...
    case 'CHAT':
      {
        ...
        const message = {
          ...
          at: now(),
          delivery: 'REMOTE_PENDING'
        };
        return withUpdated(state, {
          matchmaking: {
            ...state.matchmaking,
            chats: {
              ...state.matchmaking.chats,
              [id]: [...previous, message]
            }
          }
        });
    ...
    case 'LUDO_ROLL':
      {
        ...
        const history = winner ? [{
          ...
          won: winner === 'PLAYER',
          at: now()
        }, ...state.games.history].slice(0, 100) : state.games.history;
    ...
    case 'SEEP_PLAY':
      {
        ...
        const history = newlyFinished ? [{
          id: makeId('GAME'),
          game: 'SEEP',
          detail: `${seep.score}-${seep.opponentScore}`,
          won: seep.winner === 'PLAYER',
          at: now()
        }, ...state.games.history].slice(0, 100) : state.games.history;
    ...
    case 'SICBO_PLAY':
      {
        ...
        const history = [{
          id: makeId('GAME'),
          game: 'SICBO',
          detail: `${action.dice.join('-')} · ${action.choice}`,
          won: settlement.win,
          at: now()
        }, ...state.games.history].slice(0, 100);
        return withUpdated(state, {
          games: {
            ...state.games,
            sicbo: {
    ...
    case 'DAILY_COINS':
      {
        ...
        if (last === new Date().toDateString()) return withUpdated(state, {
          lifecycle: {
            ...state.lifecycle,
            lastError: 'DAILY COINS ALREADY CLAIMED'
          }
        });
        return withUpdated(state, {
          coins: {
            ...coinEntry(state, 55, 'DAILY 5FIVE5'),
            lastDailyClaim: now()
          }
        });
      }
    case 'TRAVEL_SAVE':
      return withUpdated(state, {
        travel: {
          ...state.travel,
          saved: [...new Set([...state.travel.saved, action.id])]
        }
      });
    case 'TRAVEL_SELECT':
      return withUpdated(state, {
    ...
    case 'TRAVEL_BOOK':
      {
        if (state.travel.bookings.some(item => item.tripId === action.trip.id && item.status !== 'CANCELLED')) {
          return withUpdated(state, {
            lifecycle: {
              ...state.lifecycle,
              lastError: 'BOOKING ALREADY EXISTS'
            }
          });
        }
        const booking = {
          ...
          place: action.trip.place,
          price: action.trip.price,
          status: 'REQUESTED',
          at: now()
        };
        return withUpdated(state, {
          travel: {
            ...state.travel,
            bookings: [booking, ...state.travel.bookings]
          }
        });
      }
    case 'TRAVEL_CANCEL':
      return withUpdated(state, {
        travel: {
          ...state.travel,
          bookings: state.travel.bookings.map(item => item.id === action.id ? {
            ...item,
            status: 'CANCELLED',
            cancelledAt: now()
          } : item)
    ...
    case 'TRAVEL_HOST':
      {
        ...
        return withUpdated(state, {
          travel: {
            ...state.travel,
            hosting: [host, ...state.travel.hosting]
          }
        });
      }
    case 'NEARBY_RESULT':
    ...
    case 'POSTER_CURRENT':
      return withUpdated(state, {
        aiPoster: {
          ...
          currentWidth: action.width ?? state.aiPoster.currentWidth,
          currentHeight: action.height ?? state.aiPoster.currentHeight,
          rotation: action.rotation ?? state.aiPoster.rotation,
          lastError: null
        }
      });
    case 'POSTER_SAVE':
      {
        const item = {
          id: makeId('POSTER'),
          uri: action.uri,
          rotation: state.aiPoster.rotation,
          at: now()
        };
        return withUpdated(state, {
          aiPoster: {
            ...state.aiPoster,
            currentUri: action.uri,
            history: [item, ...state.aiPoster.history.filter(old => old.uri !== action.uri)].slice(0, 20)
          },
          userSeed: {
            ...state.userSeed,
            profilePoster: action.uri,
            updatedAt: now()
          }
        });
      }
    ...
    case 'POST_ADD':
      {
        const post = {
          ...
          text: action.text.trim(),
          at: now(),
          author: state.userSeed.displayName
        };
        return withUpdated(state, {
          social: {
            ...state.social,
    ...
    case 'STORY_ADD':
      {
        ...
        return withUpdated(state, {
          social: {
            ...state.social,
            stories: [story, ...state.social.stories.filter(item => item.expiresAt > Date.now())].slice(0, 100)
          }
        });
      }
    case 'LIVE_TOGGLE':
      if (state.userSeed.tier !== '444') return withUpdated(state, {
        lifecycle: {
          ...state.lifecycle,
          lastError: 'ACE 444 REQUIRED'
        }
      });
      return withUpdated(state, {
        social: {
          ...state.social,
          livePreview: !state.social.livePreview
        }
      });
    case 'REPORT':
      {
        ...
        return withUpdated(state, {
          social: {
            ...state.social,
            reports: [report, ...state.social.reports]
          },
          safety: {
            ...state.safety,
            reports: [report, ...state.safety.reports]
          }
        });
      }
    case 'BLOCK':
      return withUpdated(state, {
        social: {
          ...state.social,
          blocked: [...new Set([...state.social.blocked, action.target])]
        },
        safety: {
          ...state.safety,
          blocked: [...new Set([...state.safety.blocked, action.target])]
        }
      });
    case 'CART_ADD':
      {
        const existing = state.commerce.cart.find(item => item.id === action.product.id);
        const cart = existing ? state.commerce.cart.map(item => item.id === action.product.id ? {
          ...item,
          qty: Math.min(10, item.qty + 1)
        } : item) : [...state.commerce.cart, {
          ...action.product,
          qty: 1
        }];
        return withUpdated(state, {
    ...
    case 'CART_REMOVE':
      return withUpdated(state, {
        commerce: {
          ...state.commerce,
          cart: state.commerce.cart.filter(item => item.id !== action.id)
        }
      });
    case 'CHECKOUT':
      {
        ...
        return withUpdated(state, {
          commerce: {
            ...state.commerce,
            cart: [],
            orders: [order, ...state.commerce.orders],
    ...
    case 'KAMA_RESULT':
      {
        ...
        const result = {
          id: makeId('KAMA'),
          score,
          label: score >= 80 ? 'GOLDEN ALIGNMENT' : score >= 60 ? 'STRONG ALIGNMENT' : 'GROWING ALIGNMENT',
          at: now()
        };
        return withUpdated(state, {
          kamashastra: {
            ...state.kamashastra,
            result,
            history: [result, ...state.kamashastra.history].slice(0, 100)
          }
        });
      }
    case 'RESET':
      return {
        ...createInitialMBWState(),
        hydrated: true
      };
    case 'REMOTE_PROFILE':
      return withUpdated(state, {
        auth: {
          ...state.auth,
          signedUp: true,
          phone: action.profile.whatsapp_number || state.auth.phone,
          phoneVerified: false
        },
        userSeed: {
          ...state.userSeed,
          displayName: action.profile.display_name || state.userSeed.displayName,
          updatedAt: now()
        }
      });
    case 'REMOTE_ENTITLEMENT':
      return withUpdated(state, {
        ...
        userSeed: {
          ...
          tier: action.tier,
          badge: action.badge,
          subscriptionState: action.status,
          updatedAt: now()
        }
      });
    case 'REMOTE_ENTITLEMENT_REVOKED':
      return withUpdated(state, {
        ...
        userSeed: {
          ...
          tier: '111',
          badge: 'BLACK',
          subscriptionState: action.status || 'NONE',
          updatedAt: now()
        }
      });
...
export function MBWGoldenMasterProvider({
  children
}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted) dispatch({
          type: 'HYDRATE',
          payload: decodedLegacy
        });
      } catch (error) {
        if (mounted) dispatch({
          type: 'HYDRATE',
          payload: createInitialMBWState()
        });
      }
    })();
    return () => {
  ...
  useEffect(() => {
    ...
    persistTimerRef.current = setTimeout(() => {
      encryptState(state).then(envelope => AsyncStorage.setItem(MBW_GOLDEN_MASTER_STORAGE_KEY, envelope)).then(() => AsyncStorage.removeItem(MBW_GOLDEN_MASTER_LEGACY_STORAGE_KEY)).catch(() => dispatch({
        type: 'ERROR',
        message: 'SECURE SAVE FAILED'
      }));
    }, 150);
  ...
  const requestNearby = useCallback(async () => {
    try {
      ...
      const results = defaultProfiles.map(profile => ({
        ...profile,
        distanceKm: Number(haversineKm(origin, profile).toFixed(1))
  ...
  const persistImage = useCallback(async (uri, suffix = 'jpg') => {
    await FileSystem.makeDirectoryAsync(MBW_POSTER_FOLDER, {
      intermediates: true
    });
  ...
  const pickSeedPoster = useCallback(async () => {
    try {
      ...
      dispatch({
        type: 'POSTER_ERROR',
        message: 'SEED IMAGE IMPORT FAILED'
      });
      return false;
    }
  }, [persistImage]);
  const transformPoster = useCallback(async mode => {
    ...
    try {
      ...
      if (mode === 'ROTATE') {
        ...
        if (width / height > ratio) cropWidth = Math.floor(height * ratio);else cropHeight = Math.floor(width / ratio);
        actions = [{
          crop: {
  ...
  }, [persistImage, safeDeletePoster, state.aiPoster.currentHeight, state.aiPoster.currentUri, state.aiPoster.currentWidth, state.aiPoster.history, state.aiPoster.rotation]);
  ...
  const startSeep = useCallback(async () => {
    ...
    dispatch({
      type: 'SEEP_START',
      deck: shuffleDeck(makeDeck(), randomBytes)
    });
  }, []);
  ...
  const resetAccount = useCallback(async () => {
    clearTimeout(persistTimerRef.current);
    await Promise.allSettled([AsyncStorage.removeItem(MBW_GOLDEN_MASTER_STORAGE_KEY), AsyncStorage.removeItem(MBW_GOLDEN_MASTER_LEGACY_STORAGE_KEY), SecureStore.deleteItemAsync(MBW_SECURE_KEY_NAME), FileSystem.deleteAsync(MBW_POSTER_FOLDER, {
      idempotent: true
    })]);
    dispatch({
      type: 'RESET'
  ...
  const api = useMemo(() => ({
    ...
    navigateChecked,
    resetAccount
  }), [state, verifyGate, requestNearby, pickPoster, pickSeedPoster, transformPoster, savePoster, startSeep, playSicbo, rollLudo, navigateChecked, resetAccount]);
