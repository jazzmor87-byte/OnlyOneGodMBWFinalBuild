...
export default withMBWExactVisualOS(function PathSelectionScreen({
  navigation
}) {
  ...
  return <View style={styles.root} pointerEvents="box-none">
      <View style={styles.actionRail} pointerEvents="box-none">
        <Pressable accessibilityRole="button" accessibilityLabel="MASTER OF LIFE" onPress={() => selectPath('MasterOfLife')} style={({
        pressed
      }) => [styles.action, pressed ? styles.actionPressed : null]}>
          <Text style={styles.actionText}>MASTER OF LIFE</Text>
        </Pressable>

        <Pressable accessibilityRole="button" accessibilityLabel="FULL ACCESS TO MBW ALL SECTIONS" onPress={() => selectPath('FullAccess')} style={({
        pressed
      }) => [styles.action, pressed ? styles.actionPressed : null]}>
