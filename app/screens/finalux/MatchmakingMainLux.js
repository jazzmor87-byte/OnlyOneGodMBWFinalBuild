...
export default function MatchmakingMainLux() {

  const __mbwPosterPool = getPosterPoolForScreen("app/screens/finalux/MatchmakingMainLux.js", "MatchmakingMainLux");
  const __mbwPickedPoster = pickPosterForIndex(__mbwPosterPool, 0);

    const navigation = useNavigation();
    return (
        <View style={styles.container}>
            <Text style={styles.title}>MatchmakingMainLux Screen</Text>
            <Image style={styles.poster} source={__mbwPickedPoster || require("../../assets/images/bg-wall.png")} />
            <Button title="Next" onPress={() => navigation.navigate('MatchmakingMainLux')} />
        </View>
    );
}
