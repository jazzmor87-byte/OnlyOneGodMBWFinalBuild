...
export default function merch_main() {
  const navigation = useNavigation();
  const __mbwPosterPool = getPosterPoolForScreen("app/screens/finalux/merch_main.js", "merch_main");
  const __mbwPickedPoster = pickPosterForIndex(__mbwPosterPool, 0) || require("../../assets/mbw_luxscreens/vault_gallery.png");
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.overlay}>
          <View style={styles.copy}>
            <Text style={styles.title}>MERCH MAIN</Text>
          </View>
          <View style={styles.actions}>
            <TouchableOpacity style={styles.primaryBtn} onPress={() => go(['MerchMain', 'MerchandiseScreen', 'MerchHome', 'HomeHub'])}>
              <Text style={styles.primaryText}>Continue</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.secondaryBtn} onPress={() => go(['MerchMainLux', 'MerchandiseScreen', 'HomeHub', 'MBWHome', 'RealmHome'])}>
