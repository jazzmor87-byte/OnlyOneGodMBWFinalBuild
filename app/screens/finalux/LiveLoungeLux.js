...
export default function LiveLoungeLux() {

  const __mbwPosterPool = getPosterPoolForScreen("app/screens/finalux/LiveLoungeLux.js", "LiveLoungeLux");
  const __mbwPickedPoster = pickPosterForIndex(__mbwPosterPool, 0);

    const navigation = useNavigation();
    return (
        <View style={styles.container}>
            <Text style={styles.title}>LiveLoungeLux Screen</Text>
            <Image style={styles.poster} source={__mbwPickedPoster || require("../../assets/images/bg-wall.png")} />
            <Button title="Next" onPress={() => navigation.navigate('LiveLoungeLux')} />
        </View>
    );
}
