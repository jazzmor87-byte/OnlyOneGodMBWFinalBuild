...
export default function SignupLux({ navigation, route }) {
  ...
  const { visual, panelFloatStyle, textFloatStyle, buttonFloatStyle } = useTatvaScreen("SignupLux");
    ...
    <ImageBackground source={BG} style={s.bg} resizeMode="cover">
      ...
      <ScrollView contentContainerStyle={s.wrap} keyboardShouldPersistTaps="handled">
        <Animated.View style={[s.floatCard, panelFloatStyle, { borderColor: visual.edge }]}>
          ...
          <Animated.Text style={[s.h, { color: visual.text }, textFloatStyle]}>SIGN UP</Animated.Text>
          <Animated.Text style={[s.p, textFloatStyle]}>Create your account through {accessLabel}.</Animated.Text>

          <View style={s.form}>
            <TextInput style={[s.input, { borderColor: visual.edge, color: visual.text }]} placeholder="FULL NAME" placeholderTextColor="#E9D7A6" />
          ...
          <Animated.View style={[s.fullWrap, buttonFloatStyle]}>
            <TouchableOpacity style={[s.btn, { borderColor: visual.edge }]} onPress={goNext} activeOpacity={0.9}>
              <View style={[s.btnAura, { backgroundColor: visual.buttonAura }]} pointerEvents="none" />
              <Animated.Text style={[s.t, { color: visual.text }, textFloatStyle]}>CREATE ACCOUNT</Animated.Text>
            </TouchableOpacity>
          </Animated.View>

          <TouchableOpacity onPress={() => navigation.navigate("LoginLux", { accessPath })} activeOpacity={0.9}>
            <Animated.Text style={[s.switchText, { color: visual.text }, textFloatStyle]}>ALREADY HAVE AN ACCOUNT? LOGIN</Animated.Text>
          </TouchableOpacity>
        </Animated.View>
      </ScrollView>
    </ImageBackground>
