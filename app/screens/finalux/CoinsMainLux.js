...
export default function CoinsMainLux() {

  const __mbwPosterPool = getPosterPoolForScreen("app/screens/finalux/CoinsMainLux.js", "CoinsMainLux");
  const __mbwPickedPoster = pickPosterForIndex(__mbwPosterPool, 0);

    const navigation = useNavigation();
    return (
        <View style={styles.container}>
            <Text style={styles.title}>CoinsMainLux Screen</Text>
            <Image style={styles.poster} source={__mbwPickedPoster || require("../../assets/images/bg-wall.png")} />
            <Button title="Next" onPress={() => navigation.navigate('CoinsMainLux')} />
        </View>
    );
}
