...
export default function match_extra() {
  const navigation = useNavigation();
  const __mbwPosterPool = getPosterPoolForScreen("app/screens/finalux/match_extra.js", "match_extra");
  const __mbwPickedPoster = pickPosterForIndex(__mbwPosterPool, 0) || require("../../assets/mbw_luxscreens/desired_signals.png");
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.overlay}>
          <View style={styles.copy}>
            <Text style={styles.title}>MATCH EXTRA</Text>
          </View>
          <View style={styles.actions}>
            <TouchableOpacity style={styles.primaryBtn} onPress={() => go(['match_found', 'matchmaking_main', 'MatchmakingScreen', 'MatchmakingHome', 'HomeHub'])}>
              <Text style={styles.primaryText}>Continue</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.secondaryBtn} onPress={() => go(['match_details', 'MatchmakingMainLux', 'HomeHub', 'MBWHome', 'RealmHome'])}>
