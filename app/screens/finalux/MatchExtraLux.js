...
export default function MatchExtraLux() {

  const __mbwPosterPool = getPosterPoolForScreen("app/screens/finalux/MatchExtraLux.js", "MatchExtraLux");
  const __mbwPickedPoster = pickPosterForIndex(__mbwPosterPool, 0);

    const navigation = useNavigation();
    return (
        <View style={styles.container}>
            <Text style={styles.title}>MatchExtraLux Screen</Text>
            <Image style={styles.poster} source={__mbwPickedPoster || require("../../assets/images/bg-wall.png")} />
            <Button title="Next" onPress={() => navigation.navigate('MatchExtraLux')} />
        </View>
    );
}
