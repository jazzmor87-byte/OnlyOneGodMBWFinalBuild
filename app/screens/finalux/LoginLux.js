...
export default function LoginLux() {

  const __mbwPosterPool = getPosterPoolForScreen("app/screens/finalux/LoginLux.js", "LoginLux");
  const __mbwPickedPoster = pickPosterForIndex(__mbwPosterPool, 0);

    const navigation = useNavigation();
    return (
        <View style={styles.container}>
            <Text style={styles.title}>LoginLux Screen</Text>
            <Image style={styles.poster} source={__mbwPickedPoster || require("../../assets/images/bg-wall.png")} />
            <Button title="Next" onPress={() => navigation.navigate('LoginLux')} />
        </View>
    );
}
