...
export default function host_circle() {
  const navigation = useNavigation();
  const __mbwPosterPool = getPosterPoolForScreen("app/screens/finalux/host_circle.js", "host_circle");
  const __mbwPickedPoster = pickPosterForIndex(__mbwPosterPool, 0) || require("../../assets/finalux/travel_ledger.png");
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.overlay}>
          <View style={styles.copy}>
            <Text style={styles.title}>HOST CIRCLE</Text>
          </View>
          <View style={styles.actions}>
            <TouchableOpacity style={styles.primaryBtn} onPress={() => go(['travel', 'travel_overseas', 'TravelScreen', 'TravelInbox', 'HomeHub'])}>
              <Text style={styles.primaryText}>Continue</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.secondaryBtn} onPress={() => go(['TravelOverseasLux', 'Safe_meet_atlas', 'HomeHub', 'MBWHome', 'RealmHome'])}>
