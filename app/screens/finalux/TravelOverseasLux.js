...
export default function TravelOverseasLux() {

  const __mbwPosterPool = getPosterPoolForScreen("app/screens/finalux/TravelOverseasLux.js", "TravelOverseasLux");
  const __mbwPickedPoster = pickPosterForIndex(__mbwPosterPool, 0);

    const navigation = useNavigation();
    return (
        <View style={styles.container}>
            <Text style={styles.title}>TravelOverseasLux Screen</Text>
            <Image style={styles.poster} source={__mbwPickedPoster || require("../../assets/images/bg-wall.png")} />
            <Button title="Next" onPress={() => navigation.navigate('TravelOverseasLux')} />
        </View>
    );
}
